...
public class LoadActionAddStep : MonoBehaviour
{
    ...
    private void Setup()
    {
        currentInstructionData.LoadAddOverride = AlwaysTrue;
        currentInstructionData.UnloadAddOverride = AlwaysFalse;
        currentInstructionData.OnLoadAdded.AddListener(LoadActionAdded);
    }
    ...
    private void UnFilter()
    {
        ...
        if (currentInstructionData.LoadAddOverride == AlwaysTrue)
            currentInstructionData.LoadAddOverride = AlwaysTrue;
    ...
    private void LoadActionAdded(LoadActionData actionData)
    {
        if (checkQuantity)
        {
            if ((actionData.Action as VesselLoadAction).Amount == this.quantity)
                quantityPassed = true;
        ...
        if (checkCargo)
        {
            if ((actionData.Action as VesselLoadAction).Cargo == this.cargo)
                cargoPassed = true;
        ...
        {
            if ((actionData.Action as VesselLoadAction).CompartmentIndex == this.compartmentIndex)
